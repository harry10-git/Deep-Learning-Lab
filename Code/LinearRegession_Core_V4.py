...
class RegressionModel(nn.Module):
    ...
    def forward(self, x):
        return self.linear(x) #self.w * x + self.b
...
ypred = model.linear.weight * xtest   + model.linear.bias

#Here we changed the code slightly compared to earlier version
#Because the output of linear layer is of size (batch_size, out_features)
#We have set the batch_size to 1 so ypred is of shape (1, 10)
#We need it as a list so convert matrix to list by taking y[0]
ax.plot(xtest.detach().cpu(), ypred[0].detach().cpu(), color='red')
